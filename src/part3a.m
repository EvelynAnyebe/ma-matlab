...
for i=1:k
    ...
    %train model
    model = svmtrain(training_label_vector,training_instance_matrix, '-t 0 -b 1 -q');
    ...
    LKProbability(row:numel(test_label_vector)*i,1)=test_label_vector;
    LKProbability(row:numel(test_label_vector)*i,2)=dec_values(1:numel(test_label_vector),1);
...
save LKProbability.mat LKProbability;
